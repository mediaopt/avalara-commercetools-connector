...
import { getCategoriesOfProduct, getCategoryTaxCode } from '../../client/create.client';
...
async function itemTaxCode(item: LineItem) {
  ...
  const taxCodes = await Promise.all(categories.map(async (x) => await getCategoryTaxCode(x.id)))
  const categoryTaxCode = taxCodes.find(el => el !== undefined)
  const productTaxCode = item?.variant?.attributes?.filter((attr) => attr.name === "ava-tax-code")[0]?.value;
