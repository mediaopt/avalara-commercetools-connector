...
- there is also a possibility for specifying product-level tax codes. To this edge, a product attribute with name `avatax-code` must be created. (Alternatively, you can configure the name to be whatever you want, and specify the same name in the deployment service and event application standard configuration as the value for `AVATAX_PRODUCT_ATTRIBUTE_NAME`. However, in case you do not specify it, the default value will be `avatax-code`.) It can be either unique for every variant or the same for each variant. The type of this attribute must be set to `text`. 
